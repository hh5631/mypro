import { isNotNull } from "@/vform/utils/util";
import { genVue2JS } from "@/vform/utils/vue2js-generator";
import { beautifierOpts } from "@/vform/utils/beautifierLoader";
import { genVue3JS } from "@/vform/utils/vue3js-generator";
...
function getElAttrs (widget, formConfig) {  //获取El组件属性
  ...
  return {
    ...
    appendButtonChild: !!wop.appendButton ? `<template #append><el-button size="small" class="${wop.buttonIcon}" ${!!wop.appendButtonDisabled ? 'disabled' : ''}></el-button></template>` : '',
  }
}
...
const elTemplates = {  //字段组件属性
  ...
  'button': (widget, formConfig) => {
    ...
    return `<el-button ${buttonType} ${buttonPlain} ${buttonRound} ${buttonCircle} ${buttonIcon}
            ${disabled} size="small">${widget.options.label}</el-button>`
  },
