<template>
  <div class="panel-tab__content">
    ...
    <!--字段配置侧边栏-->
    <el-drawer
      :visible.sync="fieldModelVisible"
      title="字段配置"
      :size="`${width}px`"
      append-to-body
      destroy-on-close
    >
      ...
      <!-- 底部按钮 -->
      <div class="element-drawer__button">
        <el-button size="small" @click="fieldModelVisible = false"
          >取 消</el-button
        >
    ...
    >
      ...
      <!-- <template slot="footer">
        <el-button size="small" @click="vFormVisiable = false">取 消</el-button>
        <el-button size="small" type="primary" @click="saveForm">保 存</el-button>
      </template> -->
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    //保存表单(自定义)
    saveForm(data) {
      // let widgetList = deepClone(data.widgetList)
      // let formConfig = deepClone(data.formConfig)
      // let jsonContent = JSON.stringify({ widgetList, formConfig }, null, '  ')
      // 
      this.$store.commit("flow/SET_FORM", data);
      this.vFormVisiable = false;
    },
    openVFormDialog() {
      this.vFormVisiable = true;
    },
    closeForm() {
      this.vFormVisiable = false;
    },
