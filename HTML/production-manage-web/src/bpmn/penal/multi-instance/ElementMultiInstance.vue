  ...
  <div class="panel-tab__content">
    <el-form size="mini" label-width="90px" @submit.native.prevent>
      ...
      >
        ...
        <el-form-item label="会签人员" key="collection">
          <el-select
            clearable
            v-model="dealUser"
            filterable
        ...
        <el-form-item label="完成条件" key="completionCondition">
          <el-input
            v-model="finishPercent"
            clearable
            @input="changeFinishPercent"
            placeholder="完成率百分比"
          />
        </el-form-item>
  ...
  methods: {
    ...
    changeDealUser() {
      ...
      this.loopInstanceForm.collection =
        "${camundaUtils.asList(" + this.dealUser.join(",") + ")}";
      this.updateLoopBase();
      this.loopInstanceForm.loopCardinality = this.dealUser.length + "";
      this.updateLoopCardinality(this.dealUser.length + "");
    },
    //修改完成条件
    changeFinishPercent(value) {
      this.editFlag = true;
      this.finishPercent = value.replace(/[^0-9]/g, "").replace(/^[0]+/, "");
      if (this.finishPercent > 100) {
      ...
      const completionCondition =
        ...
        this.finishPercent / 100 +
        "}";
      this.updateLoopCondition(completionCondition);
    },
    //解析回显多实例数据
    parseTheEcho() {
      if (this.loopInstanceForm.collection) {
        const loopCardinality = this.loopInstanceForm.collection
          .replace("${camundaUtils.asList(", "")
          .replace(")}", "");
        this.dealUser = loopCardinality.split(",");
      } else {
        this.dealUser = [];
      }
      //完成条件
      this.finishPercent = (
        this.loopInstanceForm.completionCondition.replace(/[^\d.]/g, "") * 100
      ).toFixed(0);
    },
