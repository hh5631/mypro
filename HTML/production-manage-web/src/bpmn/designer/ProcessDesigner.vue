<template>
  <div class="my-process-designer">
    <div
      ...
      v-if="!$slots['control-header'] && !instanceData.highLine"
    >
      ...
      <template>
        ...
        <!-- <el-button-group
          key="deploy"
          v-show="!(bpmninfo && bpmninfo.deploymentId)"
        >
          <el-button
            :size="headerButtonSize"
            :type="headerButtonType"
    ...
    <el-dialog
      ...
      destroy-on-close
    >
      <pre><code class="hljs" v-html="highlightedCode(previewType, previewResult)" /></pre>
...
export default {
  ...
  methods: {
    ...
    initDiagram(val) {
      if (val) {
        this.type = val.bpmnType;
        const { deploymentId, resourceName } = val;
        getFlowXml({ deploymentId, resourceName }).then((da) => {
          this.createNewDiagram(da);
        });
      } else {
    ...
    initModelListeners() {
      ...
      EventBus.on("commandStack.changed", async (event) => {
        try {
          ...
          let eleLength=Object.keys(this.bpmnModeler.get("elementRegistry")._elements).length
          this.$emit("module-change",eleLength)
        } catch (e) {
    ...
    /* 创建新的流程图 */
    async createNewDiagram(xml, reload) {
      // 将字符串转换成图显示出来
      //重置节点表单数据
      this.$store.commit("flow/RESET_VFORMDATA");
      ...
      try {
        ...
        //重新绘制保留processName
        if (reload) {
          const keyElement = elementRegistry.find(
            (el) => el.type === "bpmn:Process"
          );
          modeling.updateProperties(keyElement, { name: this.processName });
        }
        // 在原有流程下新建
        if (xml && this.type === 1) {
          const keyElement = elementRegistry.find(
          ...
          modeling.updateProperties(keyElement, {
            id: `Process_${new Date().getTime()}`,
          });
        }
        //处理已完成流程
        let highshapeIds = this.instanceData.highPoint || [];
        let waitshapeIds = this.instanceData.waitingToDo || [];
        highshapeIds.forEach((id) => {
          let shape = elementRegistry.get(id);
          if(shape) modeling.setColor(shape, { stroke: "green" });
        });
        //获取下一节点
        waitshapeIds.forEach((id) => {
          ...
          let nextKey = shape.businessObject.outgoing[0].targetRef.id;
          this.setNextKey(nextKey);
          if(shape) modeling.setColor(shape, { stroke: "yellow" });
        });
    ...
    importLocalFile() {
      ...
      reader.onload = function () {
        ...
        that.createNewDiagram(xmlStr, true);
      };
    },
    /* ------------------------------------------------ refs methods ------------------------------------------------------ */
    ...
    processRestart() {
      ...
      this.createNewDiagram(null, true);
    },
    elementsAlign(align) {
    ...
    // 部署
    async saveMyBpmn() {
      const { xml } = await this.bpmnModeler.saveXML({ format: true }).then();
      return xml
    },
    //预览
