<template>
  <div id="design" class="design-box">
    <!-- <my-process-palette /> -->
    <my-process-designer
...
import CustomContentPadProvider from '@/bpmn/designer/plugins/content-pad'
// 自定义左侧菜单（修改 默认任务 为 用户任务）
import CustomPaletteProvider from '@/bpmn/designer/plugins/palette'
// import xmlObj2json from '@/utils/xml2json'
// import MyProcessPalette from '@/bpmn/palette/ProcessPalette'
// 自定义侧边栏
// import MyProcessPanel from "../bpmn/process-panel/ProcessPanel";
// import {
//   deepClone,
// } from "@/vform/utils/util";
// import { addBpmn } from '@/api/process'
export default {
  name: 'Design',
  props: ['processName', 'formJson', 'processType'],
  ...
  methods: {
    // 保存流程
    async saveMyBpmn() {
      return await this.$refs.processDesigner.saveMyBpmn()
    },
    // 初始化
    initDiagram(row, type) {
      if (type) {
        this.$refs.processDesigner.initDiagram(row)
      } else {
        this.$refs.processDesigner.initDiagram()
      }
    },
    initModeler(modeler) {
      setTimeout(() => {
    ...
    getModuleLength(length) {
      this.$emit('bpmn-module-change', length)
    }
  }
}
</script>
