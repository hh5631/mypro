...
export default {
  ...
  methods: {
    ...
    //  角色选择
    handleCheckChange() {
      this.checkNodes = this.$refs.posttree.getCheckedNodes()
      if (this.checkNodes.length === this.postOptions.filter(item => item.status === '0').length) {
        this.check = true
        this.isChecked = true
      } else {
        this.check = false
    ...
    //  确定角色
    confirmRole() {
      this.$emit('checkPost', this.checkNodes)
    },
    //  角色弹框全选
    selectAll() {
      if (this.isChecked === false) {
        this.isChecked = true
        this.check = true
        const data = this.postOptions
        this.$refs.posttree.setCheckedNodes(data.filter(item => item.status === '0'))
      } else {
        this.isChecked = false
        this.check = false
        this.$refs.posttree.setCheckedNodes([])
      }
    },
    //  清空选择
    clearChoose() {
