...
static int snpnet_open ( struct net_device *netdev ) {
	...
	if ( snp->snp->Mode->State == EfiSimpleNetworkStarted ) {
		...
		DBGC(snp, "After  snp->Initialize(), mode->State = %d\n", snp->snp->Mode->State);
		DBG_PAUSE();
	...
	if ( ( rc = snpnet_rx_filters ( netdev ) ) != 0 ) {
		DBGC(netdev, "snpnet_rx_filter() error.\n");
		/* Ignore error */
	}
	...
	DBGC(netdev, "snpnet_open() complete.");
	DBG_PAUSE();
	return 0;
}
...
int snpnet_start ( struct efi_device *efidev ) {
	...
	DBGC( device, "in snpnet_start, device handle is %p", device);
	DBG_PAUSE();
	/* Open SNP protocol */
...
//					    EFI_OPEN_PROTOCOL_EXCLUSIVE )))!=0){
	...
	DBGC(device, "After open snp protocol\n");
	DBG_PAUSE();
	...
	DBGC(device, "After (possible) snp->Start(), mode->State = %d\n", mode->State);
	DBG_PAUSE();
...
// err_shutdown:
	...
	netdev_put ( netdev );
	err_alloc:
	DBG("closing protocol %s", efi_guid_ntoa ( &efi_simple_network_protocol_guid ));
	DBG_PAUSE();
