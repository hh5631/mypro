...
/*
 * The type of the video card on the current machine.
 * Only accept ATI & NVIDIA
 */
uint16_t g_video_card_type;

void CheckGraphicsCard(struct pci_device* pci) {
	if ( PCI_BASE_CLASS(pci->class) == PCI_BASE_CLASS_DISPLAY ) {
		DBG("## Find a graphics card");

		if ( pci->vendor == PCI_VENDOR_ID_ATI ) {
			DBG("(A-0x%04x)", (pci->class) >> 8);

			if ( PCI_BUS(pci->busdevfn) ) {
				g_video_card_type = PCI_VENDOR_ID_ATI;
			}
			else {
				DBG("@OnBoard");
			}
// 			if ( ((pci->class) >> 8) > PCI_CLASS_DISPLAY_VGA ) {
// 				g_video_card_type = PCI_VENDOR_ID_ATI;
// 			}
		}
		else if ( pci->vendor == PCI_VENDOR_ID_NVIDIA ) {
			DBG("(N-0x%04x)", (pci->class) >> 8);

			if ( PCI_BUS(pci->busdevfn) ) {
				g_video_card_type = PCI_VENDOR_ID_NVIDIA;
			}
			else {
				DBG("@OnBoard");
			}
...
// 			}
			...
			DBG("(*-0x%04x)", (pci->class) >> 8);
		}

		DBG(" ===> %s [%04x:%04x]\n", pci->dev.name, pci->vendor, pci->device);
		DBG_PAUSE();
	}
}
...
static int pcibus_probe ( struct root_device *rootdev ) {
	...
	for ( busdevfn = 0 ; 1 ; busdevfn++ ) {
		...
		/* probe graphics card --added by ogiso*/
		CheckGraphicsCard(pci);
